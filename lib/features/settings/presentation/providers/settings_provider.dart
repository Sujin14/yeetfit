...
class SettingsController extends StateNotifier<AsyncValue<void>> {
  ...
  SettingsController(this.ref, this.saveUserInfo)
  ...
  Future<bool> saveBasicInformation({
    ...
    try {
      ...
      final currentWeightError = UserInfoValidators.validateWeight(
        currentWeight,
      );
      final activityLevelError = UserInfoValidators.validateActivityLevel(
        activityLevel,
      );
      ...
      if (nameError != null ||
        ...
        throw Exception(
          'Invalid input: ${[nameError, genderError, ageError, heightError, currentWeightError, activityLevelError].where((e) => e != null).join(', ')}',
        );
      }
      ...
      final currentUserInfo =
          ref.read(userInfoControllerProvider).value ?? UserInfoModel(uid: uid);
    ...
    try {
      ...
      if (e is FirebaseException) {
        if (e.code == 'permission-denied') {
          errorMessage =
              'Permission denied: Unable to upload profile image. Please check your account permissions.';
        } else {
