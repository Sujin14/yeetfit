...
class ChatController extends StateNotifier<ChatState> {
  ...
  Future<void> setupChat() async {
    ...
    try {
      ...
      _messagesSubscription = getChatMessages(_chatId!).listen(
        (data) {
          state = state.copyWith(messages: data, isLoadingMessages: false);
          for (var message in data) {
            if (message.senderId != _participantId &&
                message.status != 'read') {
      ...
      _typingSubscription = getTypingStatus(_chatId!, adminId).listen(
        (typing) {
      ...
      _profileSubscription = getUserProfile(adminId).listen(
        (profile) {
          state = state.copyWith(
