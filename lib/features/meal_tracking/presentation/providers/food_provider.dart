...
import '../../domain/usecases/get_food_data.dart';
import '../../domain/usecases/get_weekly_food_data.dart';
import '../../domain/usecases/set_calorie_goal.dart';
...
final calorieGoalProvider = StateNotifierProvider.autoDispose
  ...
  (ref, userId) {
    ...
    final setCalorieGoal = ref.read(setCalorieGoalProvider);
    return CalorieGoalNotifier(ref, repository, setCalorieGoal, userId);
  },
);
...
// User info for BMR calc.
final userInfoProvider = FutureProvider.family<UserInfoModel, String>((
  ref,
  userId,
) async {
...
// Daily food items notifier for meal type.
...
// Daily calories for meal type.
final dailyCaloriesProvider = Provider.autoDispose.family<double, String>((
  ref,
  userIdAndMealType,
) {
  final foodItems =
      ref.watch(dailyFoodItemsProvider(userIdAndMealType)).value ?? [];
  return foodItems.fold(0.0, (sum, item) => sum + item.calories);
});

// Daily nutrients total.
final dailyNutrientsProvider = Provider.autoDispose
    .family<Map<String, double>, String>((ref, userId) {
      final mealTypes = [
        'Breakfast',
        'Morning Snack',
        'Lunch',
      ...
      for (final mealType in mealTypes) {
        final foodItems =
            ref.watch(dailyFoodItemsProvider('$userId|$mealType')).value ?? [];
...
// Nutrient goals based on user info.
final nutrientGoalsProvider = Provider.autoDispose
    .family<Map<String, double>, String>((ref, userId) {
      ...
      return userInfoAsync.when(
        data: (userInfo) {
          ...
          double activityFactor = userInfo.activityLevel == 'Moderately Active'
              ? 1.55
              : 1.2;
...
// Meal calorie goals based on user info.
final mealCalorieGoalsProvider = Provider.autoDispose
    .family<Map<String, double>, String>((ref, userId) {
      final userInfoAsync = ref.watch(userInfoProvider(userId));
      return userInfoAsync.when(
        data: (userInfo) {
...
// Daily calorie progress color.
final dailyCalorieProgressColorProvider = Provider.autoDispose
...
// Weekly food data by meal type.
final weeklyFoodDataProvider = FutureProvider.autoDispose
    .family<Map<String, List<FoodItem>>, String>((ref, userId) async {
      final mealTypes = [
        'Breakfast',
        'Morning Snack',
        'Lunch',
      ...
      for (final mealType in mealTypes) {
        final asyncResult = await ref
            .read(getWeeklyFoodDataProvider)
            .call(userId, mealType);
        weeklyData[mealType] = asyncResult;
      }
      return weeklyData;
    });

// Notifier for daily food items by meal type.
  ...
  Future<void> _fetchGoal() async {
    try {
      ...
      double activityFactor = userInfo.activityLevel == 'Moderately Active'
          ...
          : 1.2;
      double totalCalories = bmr * activityFactor;
      if (userInfo.goal.toLowerCase() == 'weight loss') totalCalories -= 500;
